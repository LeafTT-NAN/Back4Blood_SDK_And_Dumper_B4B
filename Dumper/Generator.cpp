...
void Generator::InitEngineCore()
{
	...
	/* For Back4Blood gobjects Decryption(essential!)*/
	InitObjectArrayDecryption([](void* ObjPtr) -> uint8* { return reinterpret_cast<uint8*>(uint64(ObjPtr) ^ 0x8375); });
	ObjectArray::Init();
	//we have override it manually,because of "gobi build" have modified something,the dumpter can't get GNames automatically
	//date:9/12/25,maybe this offset can work forever due to turtle rock gave up the updates.
	FName::Init(0x6986C80,FName::EOffsetOverrideType::GNames,true);
	Off::Init();
